...
- (void)drawEntireGraph {
    ...
    if (self.enableYAxisLabel) {
        self.YAxisLabelXOffset = 4 + [self calculateWidestLabel];
    } else {
...
- (CGFloat)labelWidthForValue:(CGFloat)value {
    ...
    NSString *labelString = [valueString stringByReplacingOccurrencesOfString:@"[0-9-]" withString:@"w" options:NSRegularExpressionSearch range:NSMakeRange(0, [valueString length])];
    return ceil([labelString sizeWithAttributes:attributes].width);
}
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        if (numberOfLabels == 1 || isfinite(self.minValue) == NO) {
